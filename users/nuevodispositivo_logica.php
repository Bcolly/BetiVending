...
function step1($basededatos){
    ...
    if (isset($_GET["ACC"]) AND $_GET["ACC"]=="COMPROBAR") {
        $error="NO";
        try{
            $dispositivos= $basededatos->query("SELECT * FROM v_dispositivo WHERE nombre='$iden'");
            $num = $dispositivos->rowCount();
        } catch (exception $e) {
            echo "No se puede la consulta 1";
            exit;
        }
        if ($num>0)  $error="enuso";
        if(strlen($iden)<6 or strlen($iden)>20) $error="tamano";
		try{
            $dispositivos= $basededatos->query("SELECT d.* FROM v_dispositivo as d, v_user as u WHERE nombre='$iden' and u.usuario='$usuario' and d.userid = u.id;");
            $num = $dispositivos->rowCount();
        } catch (exception $e) {
            echo "No se puede la consulta 1";
            exit;
        }
		if ($num>0)  $error="registrado";
        try{
            $dispositivos= $basededatos->query("SELECT * FROM v_provisional_dispo WHERE nombre='$iden'");
            $num2 = $dispositivos->rowCount();
		} catch (exception $e) {
            echo "No se puede la consulta 2";
            exit;
        }
        if ($num2>0)  $error="enuso";
        try{
            $dispositivos= $basededatos->query("SELECT d.* FROM v_provisional_dispo as d, v_user as u WHERE d.nombre='$iden' and u.usuario='$usuario' and d.userid = u.id;");
            $num2 = $dispositivos->rowCount();
        } catch (exception $e) {
            echo "No se puede la consulta 2";
            exit;
        }
        if ($num2>0)  $error="preparando";
    }
	switch ($error) {
        case "enuso":
        ...
        case "NO":
            ...
            #echo $hoy." | Caduca: ".$caduca." | usuario: ".$usuario." | Identificador: ".$iden;
			try{
                #$dispositivos= $basededatos->query("SELECT * FROM bv_dispositivos WHERE usuario='usuario'");
                $sql= $basededatos->prepare("INSERT INTO v_provisional_dispo (nombre,caduca,userid) VALUES('$iden','$caduca',
				(SELECT id FROM v_user WHERE usuario='$usuario'));");
                $sql->execute();
            } catch (exception $e) {
                echo "No se puede la consulta 1";
                echo $e->getMessage();
		...
		<form class="form-inline" role="form" action="nuevodispositivo.php" method="GET">
			<div class="form-group">
				...
				<select name='iden' class="form-control" onchange='mostrarText()'>
				  ...
				  <?php
					try{
						$dispositivos= $basededatos->query("SELECT d.* FROM v_provisional_dispo as d, v_user as u WHERE u.usuario='$usuario' and d.userid = u.id;");
					} catch (exception $e) {
						echo "No se puede la consulta 2";
						exit;
					}
...
function step2($iden, $basededatos){
	...
	if (isset($_GET["ACC"]) AND $_GET["ACC"]=="BUSCAR") {
		...
		if ($num < 1) {
			...
			echo "</div>";
			try{
                $sql= $basededatos->prepare("UPDATE v_dispositivo SET tipo='$_GET[dispositivo]' WHERE nombre='$iden';");
                $sql->execute();
            } catch (exception $e) {
                echo "No se puede la consulta 1";
                echo $e->getMessage();
...
<?php
	try{
		$dispositivos= $basededatos->query("SELECT * FROM v_maquinas as m, v_dispositivo as d 
		WHERE d.nombre='$iden' AND m.dispositivoid=d.id");
		$num = $dispositivos->rowCount();
	} catch (exception $e) {
		echo "No se puede la consulta 1";
		exit;
...
?>
	...
	<a href="<?php 
		try{
			$dispositivos= $basededatos->query("SELECT enlace FROM v_dispositivo WHERE nombre='$iden'");
		} catch (exception $e) {
			echo "No se puede la consulta 2";
			exit;
		}
...
<?php
	...
	if (isset($_GET["ACC"]) AND $_GET["ACC"]=="AÑADIR") {
		try{
			$sql = $basededatos->prepare("INSERT INTO v_maquinas (nombre, tipo, dispositivoid)
